...
import { rewriteByRules } from "@/lib/demo";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (err instanceof LlmUnavailableError) {
      return Response.json({ result: fallbackResult(text, ruleFindings, specialty), mode: "rules-only" });
    }
...
function fallbackResult(
  ...
  specialty: ReturnType<typeof getSpecialty>,
): CheckResult {
  ...
  return {
    ...
    summary:
      findings.length === 0
        ...
        : `키워드 기준으로 ${findings.length}건의 위험 표현이 있습니다. 위험 문장을 뺀 수정본을 아래에 붙여 두었습니다. OPENAI_API_KEY를 설정하면 문맥에 맞는 대체 문장까지 만듭니다.`,
    findings,
    needsPreReview: true,
    preReviewReason: "네이버·인스타그램 등 일평균 이용자 10만 명 이상 매체의 광고성 게시물은 사전심의 대상입니다.",
    rewritten: rewriteByRules(text, specialty),
  };
}
