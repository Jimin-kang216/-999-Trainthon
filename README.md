...
세 화면이 아니라 다섯 화면, 하나의 규칙 데이터.

| 화면 | 하는 일 | 규정 적용 방식 |
| --- | --- | --- |
| 광고 문구 검사 `/check` | 붙인 글을 조항별로 검사하고 위험 구절·이유·대체 문구·사전심의 대상 여부·전체 수정본을 반환 | 규칙 기반 키워드 사전 검사 + LLM 문맥 검사 병합. 키 없으면 위험 문장 제거 수정본 |
| 콘텐츠 생성 `/generate` | 프로필 사실만으로 플레이스 소개글 / 블로그 글 / 인스타 게시물 작성 | 조항을 시스템 프롬프트에 내장, 생성 후 규칙 기반 자체 검사. 키 없으면 데모 템플릿 |
| 리뷰 답변 `/review` | 플레이스 리뷰 답글 작성 | 진료 내용 재언급·개인정보·유인 표현 금지. 키 없으면 데모 템플릿 |
| 이번 주 계획 `/plan` | 지역 검색 키워드 + 7일 콘텐츠 일정 | 프로필의 동·구·진료 항목으로 조합. 외부 API 없음 |
| 한의원 프로필 `/profile` | 화면이 공유하는 사실 정보 입력 (브라우저 저장) | 프로필에 없는 사실은 생성하지 않도록 제한 |
...
`OPENAI_API_KEY`가 없어도 실행된다. 이 경우 검사·생성·리뷰가 모두 데모 모드로 동작한다(키워드 검사, 위험 문장 제거, 프로필 기반 템플릿). 키를 넣으면 같은 화면에서 LLM 문맥 검사·문장 생성으로 전환된다. 상단 칩이 `데모 모드` / `AI 연결됨`을 표시한다.
...
src/
  app/
    page.tsx                 랜딩 (문제·3단계 데모·발표 대본·조항 목록)
    check/page.tsx           광고 문구 검사
    generate/page.tsx        콘텐츠 생성
    review/page.tsx          리뷰 답변
    plan/page.tsx            지역 키워드 + 7일 계획
    profile/page.tsx         한의원 프로필
    api/check/route.ts       POST { specialtyId, channel, text } -> { result, mode }
    api/generate/route.ts    POST { profile, channel, topic } -> { result, selfCheck, mode }
    api/review-reply/route.ts POST { profile, review, rating } -> { result, mode }
    api/status/route.ts      GET -> { llm, model }
  ...
  lib/
    ...
    demo.ts                  API 키 없을 때 수정본·생성·리뷰 폴백
    local-plan.ts            지역 키워드·주간 일정
...
- **제품 완성도**: 외부 연동 없이 텍스트 입출력으로 완결. API 키가 없어도 검사·생성·리뷰·주간 계획이 끝까지 동작하는 데모 모드.
